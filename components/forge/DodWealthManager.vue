<template>
  <v-card>
    <v-card-title style="background-color: #262e37; color: #fff" class="mb-4">
      ...
      <v-spacer />
      <v-icon dark @click="$emit('cancel')"> закрыть </v-icon>
    </v-card-title>
    ...
    <v-card-text>
      ...
      <div class="mt-1" v-show="modifiers && modifiers.length > 0">
        <ul>
          <li v-for="item in modifiers">
            ...
            <v-btn v-if="item.id" icon x-small @click="removeModifier(item.id)">
      ...
      <p>
        In case some event or the GM causes some additional (or substraction
        thereoff) {{ trait.name }}, you can add it here.
      </p>
      <v-row>
        <v-col :cols="12" :md="4">
          <v-text-field
