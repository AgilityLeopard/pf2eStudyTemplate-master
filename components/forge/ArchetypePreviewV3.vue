    ...
    <v-row no-gutters>
      <v-col :cols="12" v-if="item.source">
        <p>
          ...
          <nuxt-link v-if="item.source.path" :to="item.source.path">{{
            item.source.book
          }}</nuxt-link>
          <span v-else>
            {{ item.source.book }}
      ...
      <v-col :cols="12" :sm="6" :md="6" :lg="5">
        <v-container class="caption">
          ...
          <v-row class="caption">
            ...
            <v-col style="background-color: #424242; color: white"
              >Species</v-col
            >
          ...
          <v-row>
            <v-col
              ...
              <span class="text--keyword">{{ keywords }}</span></v-col
            >
          </v-row>

          <v-row style="background-color: lightgray">
            <v-col
              ><strong>Attributes:</strong> {{ attributePrerequisites }}</v-col
            >
          </v-row>
          ...
          <v-row
            ...
            v-if="item.influence && item.influence != 0"
          >
            <v-col
              ><strong>Influence:</strong>
              {{ `${item.influence > 0 ? "+" : ""}${item.influence}` }}</v-col
            >
          </v-row>

          <v-row v-show="false">
            <v-col :cols="8" style="background-color: #f44336; color: white"
              >Suggested Attributes</v-col
            >
            <v-col :cols="3" style="background-color: #424242; color: white"
              >XP Cost</v-col
            >
          ...
          <v-row v-show="false">
            <v-col :cols="5" style="background-color: #424242; color: white"
              >Attribute</v-col
            >
            ...
            <v-col :cols="1" style="background-color: lightgray">{{
              suggestedAttributes.toughness
            }}</v-col>
            <v-col :cols="1">{{ suggestedAttributes.agility }}</v-col>
            <v-col :cols="1" style="background-color: lightgray">{{
              suggestedAttributes.initiative
            }}</v-col>
            <v-col :cols="1">{{ suggestedAttributes.willpower }}</v-col>
            <v-col :cols="1" style="background-color: lightgray">{{
              suggestedAttributes.intellect
            }}</v-col>
          ...
          <v-row
            v-show="false"
            v-if="suggestedTalents && suggestedTalents.length > 0"
          >
            <v-col :cols="12" style="background-color: #f44336; color: white"
              >Suggested Talents</v-col
            >
            <v-col :cols="12">
              {{
                suggestedTalents
                  .map(
                    (t) => `${t.name} (${t.source.key}, pg. ${t.source.page})`
                  )
                  .join(", ")
              }}
            </v-col>
...
.zigzag {
  background: linear-gradient(135deg, #fff 5px, transparent 0) 0 5px,
    linear-gradient(-135deg, #fff 5px, #f44336 0) 0 5px;
