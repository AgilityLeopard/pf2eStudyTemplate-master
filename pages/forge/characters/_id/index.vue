  ...
  <div>
    ...
    <v-container fluid class="pa-0" style="max-width: 1200px">
      <v-row justify="center" align="stretch">
            ...
            <v-row dense>
                ...
                <div class="d-flex flex-column align-center w-100 flex-wrap">
                  ...
                  <div class="d-flex align-center w-100 flex-wrap">
                    ...
                    <div class="tags d-flex flex-wrap">
                      ...
                      <v-btn small depressed class="tag-btn">
                        <v-icon left small>mdi-window-maximize</v-icon>{{ archetypeLabel }}
                      </v-btn>
                      <v-btn small depressed class="tag-btn">
              ...
              <v-col cols="12" sm="6" md="6" class="d-flex flex-wrap">
                <div class="actions d-flex flex-wrap justify-center mb-2">
                  <v-btn target="_blank" @click="handlePrint" small depressed class="action-btn">
                    Печать
                  </v-btn>
                  <v-btn :to="`/forge/characters/${characterId}/builder/setting`" small depressed class="action-btn">
                    Изменить
                  </v-btn>
                </div>
                ...
                <div class="level-xp d-flex flex-column align-center">
                  ...
                  <v-text-field dense hide-details solo flat placeholder="Опыт" class="xp-input mt-1"
                    :value="characterXp" @input="handleXpInput" ref="xpInput" />
                </div>
              </v-col>
            </v-row>
          </v-card>
        ...
        <!-- Правая карточка -->
        <v-col cols="12" md="4">
          <v-card class="glass-card pa-4 d-flex flex-column align-center stat-card" height="100%">
            <v-row no-gutters align="center" justify="start" style="width: 100%">
              <v-col cols="6" class="d-flex flex-column align-center">
                <div class="title white--text mb-2">Хиты</div>

                <div class="d-flex align-center">
                  <!-- <span class="hp-value"> {{ characterHitPointsMax() }}</span> -->
                  <div v-if="!editingHp" class="hp-value" @click="editingHp = true" :style="{
                    width: '3ch',
                    fontSize: '1.5rem',
                    textAlign: 'center',
                    color: hpColor(),
                  ...
                  <!-- Поле ввода -->
                  <v-text-field v-else v-model.number="currentHP" type="number" hide-details single-line solo flat
                    background-color="transparent" class="hp-input" :style="{
                      width: '4ch',
                      fontSize: '1.5rem',
                      textAlign: 'center',
                    }" @input="addCurrentHP" @blur="editingHp = false" @keyup.enter="editingHp = false"></v-text-field>
        ...
        <v-col cols="12" md="4">
          <v-card class="glass-card pa-3 stat-card" height="100%">
            ...
            <div v-if="activeStatuses.length > 0" class="mb-3">
              <v-tooltip bottom v-for="status in activeStatuses" :key="status.key">
                <template v-slot:activator="{ on, attrs }">
                  <v-chip small close @click:close="removeStatus(status.key)" class="ma-1" v-bind="attrs" v-on="on">
                    <span>{{ status.name }}</span>
                    <span class="ml-1" v-if="
            ...
            <div class="d-flex justify-center">
              <div class="magazine" @click="handleClick2($event, characterHeroPoints)" @contextmenu.prevent="
                handleClick2(
                  $event,
                ...
                ">
                <span v-for="n in 3" class="bullet" :class="{
          ...
          <v-dialog v-model="statusDialog" max-width="700">
            <v-card>
              <v-card-title class="headline">
                ...
                <v-spacer></v-spacer>
                <v-text-field v-model="statusSearch" append-icon="mdi-magnify" label="Поиск состояний" single-line
              ...
              <v-card-text>
                ...
                <v-tabs-items v-model="tab">
                      ...
                      <v-list-item v-for="status in activeStatuses" :key="status.key">
                        ...
                        <v-list-item-action class="action-buttons">
                          ...
                          " class="value-controls">
                            <v-btn x-small icon @click="updateStatusValue(status, -1)" :disabled="status.value <= 1">
                              <v-icon x-small>mdi-minus</v-icon>
                            </v-btn>
                            <span class="value-display mx-2">{{
                              status.value
                            }}</span>
                            <v-btn x-small icon @click="updateStatusValue(status, 1)" :disabled="status.value >= 10">
                  ...
                  <v-tab-item>
                    <v-list class="pt-3">
                      <v-list-item v-for="status in filteredStatuses" :key="status.key" @click="toggleStatus(status)"
                        :class="{ 'active-status': isStatusActive(status.key) }">
                        <v-list-item-action>
                          <v-checkbox :input-value="isStatusActive(status.key)" color="primary"></v-checkbox>
                        </v-list-item-action>
                        ...
                        <v-list-item-content>
                          <v-list-item-title>{{ status.name }}
                            <v-tooltip bottom>
                              ...
                              <span v-html="status.description"></span> </v-tooltip></v-list-item-title>
                          <!-- <v-list-item-subtitle>{{
                            status.description
                          }}</v-list-item-subtitle> -->
                          <v-list-item-subtitle v-if="statusEffects[status.key]?.hasValue"
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn @click="statusDialog = false" color="primary">Close</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        </v-col>
        ...
        <!-- 2. Armor Class -->
        <v-col cols="12" md="4">
          <v-card class="glass-card pa-3 d-flex flex-column align-center stat-card" height="100%">
            <v-row no-gutters align="center" justify="start" style="width: 100%">
              <v-col cols="4" class="d-flex justify-center align-center" style="position: relative">
                <svg viewBox="100 90 55 160" width="100" height="100">
                  <g>
                    <path fill="rgba(70,130,180,0.8)" stroke="#ffffffaa" stroke-width="1"
              ...
              <!-- Кнопки -->
              <v-col cols="8" class="d-flex stat-row" style="flex-direction: column; gap: 8px">
                <div class="stat-group">
                ...
                <div class="stat-group">
                  <v-btn small class="stat-name-btn" depressed>Стойкость:</v-btn>
        ...
        <v-col cols="12" md="4">
          <v-card class="glass-card pa-3 d-flex flex-column align-center stat-card" height="100%">
            <v-row no-gutters align="center" justify="start" style="width: 100%">
              <v-col cols="4" class="d-flex flex-column align-center" style="position: relative">
                <!-- Надпись сверху -->
                <div class="stat-title">Скорость</div>
                <div class="stat-icon-wrapper">
                  <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="75" height="75"
                    viewBox="0 0 1280.000000 1002.000000" preserveAspectRatio="xMidYMid meet">
                    <g transform="translate(0.000000,1002.000000) scale(0.100000,-0.100000)" fill="#dee2e625"
                      stroke="none">
...
133 -547 237 -779 281 -36 6 -87 16 -115 21 -54 11 -336 20 -400 13z"></path>
...
-5z"></path>
              ...
              <v-col cols="4" class="d-flex flex-column align-center" style="position: relative">
                ...
                <!-- SVG с числом внутри -->
                <div class="stat-icon-wrapper">
                  <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="80" height="72"
                    viewBox="0 0 1280.000000 863.000000" preserveAspectRatio="xMidYMid meet">
                    <g transform="translate(0.000000,863.000000) scale(0.100000,-0.100000)" fill="#dee2e625"
                      stroke="none">
                      ...
                      39 -65 72 -59 72 6 0 102 -30 214 -66z"></path>
                    </g>
                    <!-- Число внутри SVG -->
                  </svg>
              ...
              <v-col cols="4" class="d-flex flex-column align-center" style="position: relative">
                ...
                <div class="stat-icon-wrapper">
                  <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="75" height="75"
                    viewBox="0 0 1278.000000 1280.000000" preserveAspectRatio="xMidYMid meet">
                    <g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" fill="#dee2e625"
                      stroke="none">
                      <path d="M1535 12720 c-372 -7 -468 -19 -660 -81 -367 -118 -619 -382 -730
        ...
        <v-col cols="12" md="12">
          <v-card>
            ...
            > -->
              ...
              <v-tab class="caption" key="psychic-powers" :href="`#tab-psychic-powers`" v-if="psychicPowers.length > 0">
                  ...
                  <v-card style="display: flex; flex-flow: column">
                    <v-row no-gutters style="height: 100%">
                      ...
                      <v-col cols="12" md="4" class="pa-2 left-col">
                        ...
                        <!-- сюда что-то -->
                        <v-data-table :headers="skillHeaders" :items="skills" dense :items-per-page="25"
                        ...
                        <div>
                          <v-expansion-panels multiple>
                            ...
                            <v-expansion-panel v-if="explorationActions">
                              <v-expansion-panel-header>
                                <div class="d-flex align-center justify-space-between w-100">
                                  <span class="section-title">Исследование</span>
                                  <!-- <v-badge
                                    color="grey"
                              ...
                              <v-expansion-panel-content>
                                <v-list dense class="action-list">
                                  <v-list-item v-for="(action, i) in explorationActions" :key="i" class="action-item"
                            ...
                            <v-expansion-panel v-if="restActions">
                              ...
                              <v-expansion-panel-content>
                                <v-list dense class="action-list">
                                  <v-list-item v-for="(action, i) in restActions" :key="i" class="action-item"
                                    @click="openDialogAction(action)">
                                    <div class="action-title">
                                      <span class="action-symbol">{{
                                        action.symbol
                            ...
                            <v-expansion-panel v-if="otherActions">
                              ...
                              <v-expansion-panel-content>
                                <v-list dense class="action-list">
                                  <v-list-item v-for="(action, i) in otherActions" :key="i" class="action-item"
                                    @click="openDialogAction(action)">
                                    <div class="action-title">
                                      <span class="action-symbol">{{
                                        action.symbol
              ...
              <v-tab-item class="my-tab-item" key="abilities-talents" :value="`tab-abilities-talents`">
                <div class="pa-2">
                  ...
                  <div style="height: 505px; overflow-y: auto">
                    <!-- species < abilities -->
                    <div v-show="abilitySection.selection === 'наследие' ||
                      ...
                      <div v-for="ability in speciesAbilities" :key="ability.name" class="caption mb-2">
                        ...
                        <div v-html="ability.effect" />

                        <div v-if="ability.selectedOptions" v-for="selectedOption in ability.selectedOptions"
                          class="ml-1 pl-2" style="border-left: solid 3px lightgrey">
                          <strong>{{ selectedOption.name }}</strong>
                          <div v-if="selectedOption.snippet">
                            <p class="mb-1" v-html="selectedOption.snippet
                              "></p>
                          </div>
                          <div v-else v-html="selectedOption.description
                            "></div>
                        </div>
                      </div>
                      <div v-if="speciesAbilities.length === 0" align="center" class="mt-2 mb-2">
                        <em>Нет особенностей? Выберите наследие и все
                          увидите!</em>
                      </div>
                    </div>

                    <!-- archetype < abilities -->
                    <div v-show="['все', 'класс'].some(
                      ...
                      <div v-for="ability in archetypeAbilities" :key="ability.key" class="caption mb-2">
                        <strong>{{ ability.name }}</strong>
                        <em v-if="ability.source"> • {{ ability.source }}</em>
                        <em v-if="ability.level"> {{ ability.level }}</em>
                        <div v-if="ability.snippet">
                          <p class="mb-1" v-html="ability.snippet"></p>
                        </div>
                        <div v-else v-html="ability.description"></div>
                    ...
                    <div v-show="abilitySection.selection === 'предыстория' ||
                      (abilitySection.selection === 'все' &&
                      ...
                      <div v-for="ability in ascensionAbilities" :key="ability.name" class="caption mb-2">
                        ...
                        <div v-if="ability.selectedOption" class="ml-1 pl-2" style="border-left: solid 3px lightgrey">
                          ...
                          <div v-if="ability.selectedOption.snippet">
                            <p class="mb-1" v-html="ability.selectedOption.snippet"></p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div v-show="['все', 'черты'].some(
                      ...
                      <div v-if="talents.length > 0" v-for="talent in talents" :key="talent.name" class="caption mb-2">
                        ...
                        <span v-if="talent.type && talent.level">
                          <em v-if="talent.category"> • {{ talent.category }}</em>

                          <em v-if="talent.level"> • {{ talent.level
                          }}</em>
                        </span>
                        <div v-if="talent.snippet">
                          <p class="mb-1" v-html="talent.snippet"></p>
                        </div>
                        <div v-else v-html="talent.description"></div>

                        <div v-if="false" class="mt-1 mb-1 ml-1 pl-2" style="
                            flex-wrap: wrap;
                            display: flex;
                    ...
                    <div v-show="abilitySection.selection === 'другое' ||
                      (abilitySection.selection === 'все' &&
                      ...
                      <div v-for="ability in otherAbilities" :key="`${ability.id ? ability.id : ability.name}`"
                        class="caption mb-3">
                        <strong>{{ ability.name }}</strong>
                        <em v-if="ability.source"> • {{ ability.source }}</em>
                        ...
                        <div v-if="ability.selectedOption" class="ml-1 pl-2 mt-1"
              ...
              <v-tab-item class="my-tab-item" key="psychic-powers" :value="`tab-psychic-powers`">
                ...
                <div v-if="!characterArchetype?.spellProgression" align="center" class="mt-2 mb-2">
              ...
              <v-tab-item class="my-tab-item" key="objectives" :value="`tab-objectives`">
                <div class="pa-2">
                  <v-chip-group mandatory active-class="red--text" v-model="descriptionSection.selection">
                    <v-chip label small v-for="item in [`Все`, `Языки`, `Трейты`, `Заметки`]" :key="item.toLowerCase()"
                  ...
                  <div style="height: 505px; overflow-y: auto">
                    ...
                    <div v-show="['все', 'трейты'].some(
                      ...
                      <div v-for="keyword in keywords" :key="keyword.name" class="caption">
                    ...
                    <div v-show="['все', 'заметки'].some(
                      ...
                      <div v-if="characterNotesShowEditor">
                        <v-textarea v-model="characterNotesEditorModel"
                          hint="Используй markdown, например **жирный** _курсив_" persistent-hint filled
