  ...
  <v-row justify="center">
    ...
    <v-tabs centered grow color="red">
      ...
      <v-tab-item class="my-tab-item" key="tab-weapon" :value="`tab-weapon`">
        <v-col :cols="12">
          <v-btn outlined block class="mt-2" color="success" @click="OpenWeaponSearch()">
            Добавить оружие
          </v-btn>

          <v-data-table :headers="headers" :items="characterWeapon" :search="searchQuery"
            :page.sync="paginationWeapon.page" hide-default-footer @page-count="paginationWeapon.pageCount = $event"
          ...
          <div class="text-center pt-2">
            <v-pagination v-model="paginationWeapon.page" :length="paginationWeapon.pageCount" />
          </div>
          <!-- Доспехи -->
              ...
              <v-card-text v-if="Weapon" class="pt-4">
                <v-row>
                  ...
                  <v-col cols="6" sm="6">
                    ...
                    <span>Руна Разящая:
                      <v-btn outlined small color="primary" @click="dialogStriking = true">
                        {{ strikingLabel }}
                      </v-btn>
                    </span>
                    <!-- Диалог выбора -->
                    <v-dialog v-model="dialogStriking" max-width="1400px">
                      <v-card>
                        <v-card-title class="headline">Выбор разящей руны</v-card-title>
                        <v-card-text>
                          <v-row>
                            <v-col cols="6" sm="4">
                              <!-- <v-select
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in weaponRuneStriking" :key="rune.key" :value="rune.key"
                                  @click="localStriking = rune.key" :class="{
                                  ...
                                  <v-list-item-icon v-if="localStriking === rune.key">
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogStriking = false">
                            Отмена
                          </v-btn>
                  ...
                  <v-col cols="6" sm="6">
                    ...
                    <!-- Кнопка, открывающая диалог -->
                    <span>Руна Мощи:
                      <v-btn outlined small color="primary" @click="dialogPotency = true">
                        {{ potencyLabel }}
                      </v-btn>
                    </span>
                    ...
                    <v-dialog v-model="dialogPotency" max-width="1400px">
                      <v-card>
                        <v-card-title class="headline">Выбор руны Мощи</v-card-title>
                        <v-card-text>
                          <v-row>
                            <v-col cols="6" sm="4">
                              <!-- <v-select
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in weaponRunePotency" :key="rune.key" :value="rune.key"
                                  @click="localPotency = rune.key" :class="{
                                  ...
                                  <v-list-item-icon v-if="localPotency === rune.key">
                                    <v-icon color="success">mdi-check</v-icon>
                                  </v-list-item-icon>
                                </v-list-item>
                              </v-list>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogPotency = false">
                            Отмена
                          </v-btn>
                ...
                <v-row>
                  <v-col cols="6" sm="6">
                    <span>Руна Свойств:
                      <v-btn outlined small color="primary" @click="dialogProperty = true">
                        Выберите руны
                      </v-btn>
                    </span>
                    <v-row>
                    ...
                    <v-dialog v-model="dialogProperty" max-width="1400px">
                      <v-card>
                        <v-card-title class="headline">Выбор руны Свойств</v-card-title>
                        <v-card-text>
                          <v-row>
                            <v-col cols="6" sm="4">
                              <!-- <v-select
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in PropertyRune" :key="rune.key"
                                  @click="toggleProperty(rune.key)" :class="{
                                  ...
                                  <v-list-item-icon v-if="localProperty.includes(rune.key)">
                                    <v-icon color="success">mdi-check</v-icon>
                                  </v-list-item-icon>
                                </v-list-item>
                              </v-list>
                            ...
                            <v-col cols="6" sm="8">
                              <div>
                                <CardItem v-if="selectedRune" :item="PropertyItem(selectedRune)"
                                  :wargearPrice="wargearPrice" />
                              </div>
                            </v-col>
                          </v-row>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogProperty = false">
                            Отмена
                          </v-btn>
      ...
      <v-tab-item class="my-tab-item" key="tab-armor" :value="`tab-armor`">
        ...
        <v-data-table :headers="headersArmor" :items="characterArmour" :search="searchQuery"
          :page.sync="paginationArmor.page" show-expand item-key="id" hide-default-footer
          @page-count="paginationArmor.pageCount = $event">
          <template v-slot:no-data> Нет предметов </template>

          <template v-slot:item.nameGear="{ item }">
            <v-row><span>{{ item.name }}</span></v-row>
            <v-row>
              <div>
          ...
          <template v-slot:item.delete="{ item }">
            <v-btn outlined x-small color="info" @click="openArmourSettings(item)">
              <v-icon left> edit </v-icon>
            </v-btn>
          </template>
        ...
        <div class="text-center pt-2">
          <v-pagination v-model="paginationArmor.page" :length="paginationArmor.pageCount" />
        </div>

        <v-col :cols="12">
          <v-dialog v-model="armorEditorDialog" :value="Armor" width="1000px" scrollable
            :fullscreen="$vuetify.breakpoint.xsOnly">
            <v-card>
              ...
              <v-card-text v-if="Armor" class="pt-4">
                <v-row>
                  <v-col cols="6" sm="6">
                    <!-- Кнопка, открывающая диалог -->
                    <span>Руна Стойкости:
                      <v-btn outlined small color="primary" @click="dialogResilientArmor = true">
                        {{ ResilientArmorLabel }}
                      </v-btn>
                    </span>
                    <!-- Диалог выбора -->
                    <v-dialog v-model="dialogResilientArmor" max-width="1400px">
                      <v-card>
                          ...
                          <v-row>
                            <v-col cols="6" sm="4">
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in armourRuneResilent" :key="rune.key" :value="rune.key"
                                  @click="localResilientArmor = rune.key" :class="{
                                    'selected-rune':
                                  ...
                                  <v-list-item-icon v-if="localResilientArmor === rune.key">
                                    <v-icon color="success">mdi-check</v-icon>
                                  </v-list-item-icon>
                                </v-list-item>
                              </v-list>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogResilientArmor = false">
                            Отмена
                          </v-btn>
                          <v-btn color="success" @click="saveResilientArmorSelection">
                            Сохранить
                          </v-btn>
                        </v-card-actions>
                      </v-card>
                  ...
                  <v-col cols="6" sm="6">
                    <!-- Кнопка, открывающая диалог -->
                    <span>Руна Мощи:
                      <v-btn outlined small color="primary" @click="dialogPotencyArmor = true">
                        {{ PotencyArmorLabel }}
                      </v-btn>
                    </span>
                    <!-- Диалог выбора -->
                    <v-dialog v-model="dialogPotencyArmor" max-width="1400px">
                      <v-card>
                          ...
                          <v-row>
                            <v-col cols="6" sm="4">
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in armourRunePotency" :key="rune.key" :value="rune.key"
                                  @click="localPotencyArmor = rune.key" :class="{
                                    'selected-rune':
                                  ...
                                  <v-list-item-icon v-if="localPotencyArmor === rune.key">
                                    <v-icon color="success">mdi-check</v-icon>
                                  </v-list-item-icon>
                                </v-list-item>
                              </v-list>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogPotencyArmor = false">
                            Отмена
                          </v-btn>
                          <v-btn color="success" @click="savePotencyArmorSelection">
                            Сохранить
                          </v-btn>
                        </v-card-actions>
                      </v-card>
                ...
                <v-row>
                  <v-col cols="6" sm="6">
                    <span>Руна Свойств:
                      <v-btn outlined small color="primary" @click="dialogPropertyArmor = true">
                        Выберите руны
                      </v-btn>
                    </span>
                    <v-row>
                      <v-chip class="mt-2" v-for="pr in PropertyArmor" color="blue">
                    ...
                    <v-dialog v-model="dialogPropertyArmor" max-width="1400px">
                      <v-card>
                          ...
                          <v-row>
                            <v-col cols="6" sm="4">
                              ...
                              <v-list dense class="rune-list">
                                ...
                                <v-list-item v-for="rune in PropertyArmorRune" :key="rune.key"
                                  @click="togglePropertyArmor(rune.key)" :class="{
                                    'selected-rune':
                                  ...
                                  <v-list-item-icon v-if="localPropertyArmor.includes(rune.key)">
                                    <v-icon color="success">mdi-check</v-icon>
                                  </v-list-item-icon>
                                </v-list-item>
                              </v-list>
                            ...
                            <v-col cols="6" sm="8">
                              <div>
                                <CardItem v-if="selectedRuneArmor" :item="PropertyArmorItem(selectedRuneArmor)"
                                  :wargearPrice="wargearPrice" />
                              </div>
                            </v-col>
                          </v-row>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text color="error" @click="dialogPropertyArmor = false">
                            Отмена
                          </v-btn>
                          <v-btn color="success" @click="savePropertyArmorSelection">
                            Сохранить
                          </v-btn>
                        </v-card-actions>
                      </v-card>
      ...
      <v-tab-item class="my-tab-item" key="tab-consumable" :value="`tab-consumable`">


        <v-btn outlined block color="success" class="mt-2" @click="ConsumableSearchDialog = true">
          Добавить расходники
        </v-btn>

        <v-data-table :headers="headersConsumable" :items="characterConsumable" :search="searchQuery"
          :page.sync="paginationCon.page" show-expand item-key="id" hide-default-footer
        ...
        <div class="text-center pt-2">
          <v-pagination v-model="paginationCon.page" :length="paginationCon.pageCount" />
        </div>
      ...
      <v-tab-item class="my-tab-item" key="tab-gear" :value="`tab-gear`">
        ...
        <div v-for="group in groupedGear" :key="group.header" class="gear-section">
          ...
          <div class="table-container">
            <v-data-table :headers="headersConsumable" :items="group.items" :search="searchQuery"
    ...
    <v-dialog v-model="moneyEditorDialog" width="600px" scrollable :fullscreen="$vuetify.breakpoint.xsOnly">
      <v-card>
        <v-card-title style="background-color: #262e37; color: #fff">
        ...
        <v-card-text class="pt-4">
          <v-row>
            <v-col cols="6" sm="6">
              <v-text-field label="Установить количество монет" dense required v-model="gp" outlined></v-text-field>
            </v-col>
            <v-col cols="6" sm="6">
              <v-btn small center color="info" @click="changeMoney()">Установить</v-btn>
            </v-col>
          </v-row>

          <v-row>
            <v-col cols="6" sm="6">
              <v-text-field label="Прибавить количество монет" dense required v-model="gpAd" outlined></v-text-field>
            </v-col>
            <v-col cols="6" sm="6">
              <v-btn small right color="info" @click="adjustMoney()">Скорректировать</v-btn>
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
...
export default {
  ...
  computed: {
    ...
    RuneWeaponProperty() {
      return this.wargearList.filter(
        s =>
    ...
    // Для получения снаряжения героя
    // -----------------------------
    characterWargear() {
  ...
  methods: {
    ...
    saveWeapon() {
      ...
      if (this.PotencyCap(this.Potency) < this.Property.length) {
        ...
        this.$store.commit('characters/updateCharacterWargear', { id: this.characterId, damage: dice, property: Property, striking: this.Striking, potency: this.Potency, gear: this.Weapon, armor: false, notEnc: false });
