...
function loadAllJSONFromFolder(folder) {
    ...
    const wantedFiles = ['ancestry.json'];
    ...
    files.forEach(file => {
        ...
        const modified = json.map(item => ({
            ...item,
            trait: translateTraits(item.trait),
            // previewText: convertDescription(item.previewText || ""),
            description: convertDescription(item.Description[0]?.about || ""),
            ...sourceMod(item.source || sourceKey),
        }));
...
function translateTraits(traits = []) {
    ...
    return traits.map(t => {
        ...
        return traitMap[t.toLowerCase()] ?? t;
    });
}
